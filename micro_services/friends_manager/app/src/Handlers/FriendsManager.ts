...
function AddFriendRequest(RMqRequest: RabbitMQRequest): RabbitMQResponse {
    ...
    {
        const query = db.persistent.prepare(`SELECT REQ_ID FROM '${requests_table_name}' WHERE ( from_uid = ? AND to_uid = ? ) OR ( from_uid = ? AND to_uid = ? ) ;`)
        const result = query.get(RMqRequest.JWT.sub, RMqRequest.message, RMqRequest.message, RMqRequest.JWT.sub) as RequestsModel;
        if (result) {
    ...
    {
        const query = db.persistent.prepare(`INSERT INTO '${requests_table_name}' ( REQ_ID , from_uid , to_uid ) VALUES( ? , ? , ? );`);
        const result = query.run(crypto.randomUUID(), RMqRequest.JWT.sub, RMqRequest.message);
        if (result.changes !== 1)
            throw `AddFriend(): database error`;
...
function AcceptFriendRequest(RMqRequest: RabbitMQRequest): RabbitMQResponse {
    ...
    var request;
    {
        const query = db.persistent.prepare(`SELECT * FROM '${requests_table_name}' WHERE REQ_ID = ? AND to_uid = ? ;`);
        request = query.get(RMqRequest.message, RMqRequest.JWT.sub) as (RequestsModel | undefined);
        if (!request) {
    ...
    {
        const query = db.persistent.prepare(`DELETE FROM '${requests_table_name}' WHERE REQ_ID = ? ;`);
    ...
    {
        const get_query = db.persistent.prepare(`SELECT friends FROM '${friends_table_name}' WHERE UID = ? ;`)
        const insert_query = db.persistent.prepare(`INSERT INTO '${friends_table_name}' ( UID , friends ) VALUES( ? , ? );`)
...
function DenyFriendRequest(RMqRequest: RabbitMQRequest): RabbitMQResponse {
    ...
    var request;
    {
        const query = db.persistent.prepare(`SELECT * FROM '${requests_table_name}' WHERE REQ_ID = ? ;`);
    ...
    // Remove request from db
    {
        ...
        const result = query.run(request.REQ_ID);
        if (result.changes !== 1)
            throw `DenyFriendRequest(): database error`;
    }
...
function RemoveFriend(RMqRequest: RabbitMQRequest): RabbitMQResponse {
    ...
    // remove friend from our friend list
    {
        ...
        const updateQuery = db.persistent.prepare(`UPDATE '${friends_table_name}' SET friends = ? WHERE UID = ? ;`);
