...
export const resetMatch = document.getElementById(
  'resetMatch'
) as HTMLButtonElement;
export const winnerText = document.getElementById(
  'winnerText'
...
export const tournment = document.getElementById(
  'tournment'
) as HTMLButtonElement;
export const aiRematch = document.getElementById(
  'resetMatchAi'
) as HTMLButtonElement;
...
export async function getNewMatch(type: number): Promise<string> {
  let res = await fetch(
    `https://transcendence.fr/api/match/create?match_type=${type}`,
...
export async function sendWinOrLose(uid: string, win: boolean): Promise<void> {
  ...
  if (!win) status = 'https://transcendence.fr/api/match/lose';
...
export async function gameMenu() {
  const UsernameQuery = await fetch(
    'https://transcendence.fr/api/user/info?uid=me'
  );
  const UsernameQueryJson = (await UsernameQuery.json()) as {
    username: string;
  };
