...
import { gameRunning, startGame, games } from './game';
...
export async function startTournment() {
  try {
    ...
    if (tempName.p2 === '' || tempName.p3 === '' || tempName.p4 === '') await getParticepantNames();
...
async function firstRound(event: KeyboardEvent) {
  ...
  if (!gameRunning) {
    ...
    if (res === 3) (tempName.winners[0] = tempName.p1), (tb2.style.opacity = '0.3');
    else if (res === 2) (tempName.winners[0] = tempName.p2), (tb1.style.opacity = '0.3');
...
async function secondRound(event: KeyboardEvent) {
  ...
  if (!gameRunning && tempName.winners[0]) {
    res = await startGame(event);
    if (res === 3) (tempName.winners[1] = tempName.p3), (tb4.style.opacity = '0.3');
    else if (res === 2) (tempName.winners[1] = tempName.p4), (tb3.style.opacity = '0.3');
...
async function finnalRound(event: KeyboardEvent) {
  ...
  if (!gameRunning && tempName.winners[1]) {
    res = await startGame(event);
    if (res === 3) {
      (tempName.winners[2] = tempName.winners[0]),
        (tb5.style.opacity = '0.3'),
        (tb4.style.opacity = '0.3'),
        (tb3.style.opacity = '0.3');
      await sendWinOrLose(matchUID, true);
    } else if (res === 2) {
      (tempName.winners[2] = tempName.winners[1]),
        (tb6.style.opacity = '0.3'),
        (tb2.style.opacity = '0.3'),
        (tb1.style.opacity = '0.3');
...
export function getParticepantNames(): Promise<void> {
  return new Promise((resolve: (value: void) => void) => {
    ...
    function handleSubmit(e: MouseEvent) {
      e.preventDefault();
      if (inputName2.value === '' || inputName3.value === '' || inputName4.value === '') {
        error2.innerText = 'All participants should have a username';
        error2.style.display = 'block';
      } else if (
        inputName2.value.length > 10 ||
