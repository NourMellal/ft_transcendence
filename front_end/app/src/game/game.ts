import { tournBoard } from './tournment';
...
export function startGame(e: KeyboardEvent): Promise<number> {
  return new Promise((resolve: (value: number) => void) => {
    ...
    if (e.code === 'Space' && players.player1 < 3 && players.player2 < 3) {
      tournBoard.style.display = 'none';
      document.removeEventListener('keyup', startGame);
      gameRunning = true;
      starttext.style.display = 'none';

      if (games.gametype === 'tournment') gameLoopTournment(resolve);
      else if (games.gametype === '1v1' || games.gametype === 'AI') gameLoop1v1(resolve);
      else if (games.gametype === '2v2') gameLoop2v2(resolve);
    }
  });
}
...
function gameLoop1v1(resolve: (value: 2 | 3) => void) {
  ...
  if (gameRunning) {
    upDatePaddle1();
    if (games.gametype === '1v1') upDatePaddle2();
    else if (games.gametype === 'AI') upDatePaddleAi();
    upDateBall();
    upDatePlayresScore();
...
function upDateBall() {
  ...
  if (games.gametype === '2v2') {
    if (
      ballX - ballRect.width / 2 <= paddle3XY.right - playGroundrect.left &&
      ...
      ballX + ballRect.width / 2 >= paddle4XY.left - playGroundrect.left &&
  ...
  if (
    ballY + ballRect.height / 2 >= playGroundrect.bottom - playGroundrect.top ||
    ballY - ballRect.height / 2 <= 0
  )
    ballSpeedY *= -1;
  if (
    ballX - ballRect.width / 2 <= paddle1XY.right - playGroundrect.left &&
    ...
    ballX + ballRect.width / 2 >= paddle2XY.left - playGroundrect.left &&
...
function resetGame() {
  ...
  if (players.player1 < 3 && players.player2 < 3) document.addEventListener('keyup', startGame);
}
