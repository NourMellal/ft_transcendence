import { startTournment, reTournment } from './tournment';
import { quit } from './quit';
import { game1v1 } from './game1v1';
import { startAiGame } from './ai';
import { startGame2v2, reMatch2v2 } from './game2v2';
import { resizeGame, paddel3, paddel4 } from './game';
...
export async function getNewMatch(type: number): Promise<string> {
  let res = await fetch(`https://transcendence.fr/api/match/create?match_type=${type}`, {
    method: 'POST',
    body: new FormData(),
  });
  if (!res.ok) {
